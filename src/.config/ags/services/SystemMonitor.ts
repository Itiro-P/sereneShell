...
class SystemMonitorClass {
    ...
    public constructor() {
        ...
        this._batteryLifeLabel = createComputed(() => {
            let prefix = this._batteryPercentage(bp => Math.round(bp * 100) + "% ");
            if(this._batteryCharging()) {
                return prefix() + `Carregando: ${formatTimeVerbose(createBinding(this._battery, "timeToFull")())} restante(s)`;
            } else {
                return prefix() + `Descarregando: ${formatTimeVerbose(createBinding(this._battery, "timeToEmpty")())} restante(s)`;
            }
        });
