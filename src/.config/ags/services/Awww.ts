...
export namespace Awww {
  ...
  class Manager {
    ...
    public async setWallpaper(
      ...
      if (options.transitionAngle)
        command += ` --transition-angle ${options.transitionAngle}`;
      if (options.transitionDurantion)
        command += ` --transition-duration ${options.transitionDurantion}`;
      if (options.transitionPos)
        command += ` --transition-pos ${options.transitionPos}`;
      if (options.transitionStep)
        command += ` --transition-step ${options.transitionStep}`;
      if (options.transitionType)
        command += ` --transition-type ${options.transitionType}`;
      ...
      try {
        await Promise.all([
          execAsync(`matugen image "${path}" --source-color-index 0`),
          execAsync(command),
        ]);
        await execAsync(
          `sass ${pathService.stylesDir}/index.scss ${pathService.stylesDir}/output.css`,
        ).then(() => {
